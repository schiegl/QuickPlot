...
node :: Int -> String -> VisJSON
node id' label = [vis|{ id' : #{ id' }, label : #{ label } }|]

edge :: Int -> Int -> VisJSON
edge from to = [vis|{ from : #{ from }, to : #{ to } }|]

network :: [VisJSON] -> [VisJSON] -> VisJSON
network nodes edges = [vis|{
      nodes : #{ nodes }
    , edges : #{ edges }
}|]
